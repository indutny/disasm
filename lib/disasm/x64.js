...
X64.prototype.readOpcode = function readOpcode(prefixes) {
  ...
  var b = this.readUInt8();
  if (b === 0x38) {
    var res = X64.opcodeTable.three1[this.readUInt8()];
    assert(res, 'Opcode not found: f,38,' + b.toString(16));
    return res;
  }
  if (b === 0x3a) {
    var res = X64.opcodeTable.three2[this.readUInt8()];
...
X64.prototype.getImmediate = function getImmediate(operand, rex, index) {
  if (operand.size === 'z')
...
X64.prototype.readModrm = function readModrm(rex, opcode, res, b) {
  ...
  if (opcode.r)
    ...
    res[opcode.imm.index] = this.getImmediate(opcode.imm, rex, reg);
